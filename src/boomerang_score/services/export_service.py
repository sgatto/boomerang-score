...
class ExportService:
    ...
    def load_csv(self, filename: str) -> dict:
        ...
        return {"title": title, "participants": participants_data}

    def export_pdf_full_list(
        self, filename: str, participant_order: Optional[list[Any]] = None
    ):
        """
    ...
    def export_scoresheet(
        ...
        circles_data = self._distribute_entries_to_circles(
            participant_order, num_circles, sort_method
        )
        ...
        if is_docx:
            self._export_scoresheet_docx(
                filename, event, circles_data, header_rows, col_widths
            )
        elif is_pdf:
            self._export_scoresheet_pdf(
                filename, event, circles_data, header_rows, num_empty_cols, col_widths
            )
        else:
            raise ValueError("Filename must end with .pdf or .docx")
    ...
    def _distribute_entries_to_circles(
        self, participant_order: list[str], num_circles: int, sort_method: str
    ):
        ...
        if sort_method == "Rank":
            ...
            entries_per_circle = max(
                1, (total_entries + num_circles - 1) // num_circles
            )
            for idx, p_id in enumerate(participant_order):
    ...
    def _export_scoresheet_pdf(
        self, filename, event, circles_data, header_rows, num_empty_cols, col_widths
    ):
    ...
    def _export_scoresheet_docx(
        self, filename, event, circles_data, header_rows, col_widths
    ):
    ...
    def _make_logo(self, logo_path: Optional[str]):
    ...
    def _build_participant_info_table(self, participant, label_style, text_style):
    ...
    def _build_discipline_table(self, participant):
    ...
    def _build_participant_page(
        ...
        logo = self._make_logo(logo_path)
        if logo:
            logo.hAlign = "CENTER"
        ...
        elements.append(
            self._build_participant_info_table(participant, label_style, text_style)
        )
        elements.append(Spacer(1, 30))
        elements.append(self._build_discipline_table(participant))
        return elements

    def _export_individual_pdf(
        self, filename: str, participant_order: list[str], logo_path: Optional[str]
    ):
        ...
        styles, title_style, h2_style, label_style, text_style = self._build_pdf_styles()
