...
class TestAddParticipant:
    ...
    def test_add_participant(self, service, competition):
        participant = service.add_participant("John Doe", 1, {"acc": 50.0, "aus": 90.0, "mta": 30.0})
    ...
    def test_add_participant_calculates_total(self, service, competition):
        service.add_participant("John", 1, {"acc": 50.0, "aus": 90.0})
    ...
    def test_add_multiple_participants_calculates_ranks(self, service, competition):
        service.add_participant("John", 1, {"acc": 50.0})
        service.add_participant("Jane", 2, {"acc": 75.0})  # Better score (higher)
        service.add_participant("Bob", 3, {"acc": 40.0})   # Worse score (lower)
...
class TestUpdateParticipant:
    ...
    def test_update_result(self, service, competition):
        service.add_participant("John", 1, {"acc": 50.0})
        service.update_participant_result(1, "acc", 45.0)
    ...
    def test_update_result_recalculates_points(self, service, competition):
        service.add_participant("John", 1, {"acc": 50.0})
        old_points = competition.get_participant(1).get_points("acc")

        service.update_participant_result(1, "acc", 45.0)
        new_points = competition.get_participant(1).get_points("acc")

        assert new_points != old_points

    def test_update_result_recalculates_ranks(self, service, competition):
        service.add_participant("John", 1, {"acc": 50.0})
        service.add_participant("Jane", 2, {"acc": 45.0})
        ...
        service.update_participant_result(1, "acc", 80.0)
...
class TestActiveDisciplines:
    ...
    def test_changing_active_disciplines_recalculates_totals(self, service, competition):
        ...
        service.add_participant("John", 1, {"acc": 50.0, "aus": 90.0, "mta": 30.0})
